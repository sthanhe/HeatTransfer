...
function alpha=grewal(d_p,rho_p,FG,d_tube,pitch,T)
    persistent g 
    if isempty(g)
        g=9.81;
    end
    ...
    Nu_T=47.*(1-eps);
    Nu_T=Nu_T.*(w.*d_tube.*eta_g./(d_p.^3.*rho_p.*g)).^0.325;
    Nu_T=Nu_T.*(rho_p.*SiO2.c_p(T).*d_tube.^1.5*sqrt(g)./lambda_g).^0.23;
